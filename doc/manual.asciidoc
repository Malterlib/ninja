...
`$n` followed by a newline:: escape the newline and add a new line
...
`environment`:: _(Available since Ninja 1.12 and only when
  `ninja_required_version >= 1.12`)_
  append these environment variables when launching
  processes. See <<ref_environment,environment blocks>> discussion.

`override_environment`:: _(Available since Ninja 1.12 and only when
  `ninja_required_version >= 1.12`)_
...
`command_cwd`:: _(Available since Ninja 1.12 and only when
  `ninja_required_version >= 1.12`)_
...
[[ref_command_raw]] `command_raw`:: _(Available since Ninja 1.12 and only when
  `ninja_required_version >= 1.12`)_ the command will be executed directly with
...
Example usage:
----
environment = PATH=C:\\Executables$n
MY_ENV_VAR=My Value

rule link
  command = link.exe /OUT$out $in
  environment = LIB=C:\\Libraries$n
