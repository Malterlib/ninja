...
with a different command line or environment variables than the build files
specify (i.e., the command line changed) and knows to rebuild the file.
...
Some variables are significant when declared in the outermost file scope.
...
`environment`:: _(Available since Ninja 1.14 and only when
  `ninja_required_version >= 1.14`)_
  append these environment variables when launching
  processes. See <<ref_environment,environment blocks>> discussion.

`override_environment`:: _(Available since Ninja 1.14 and only when
  `ninja_required_version >= 1.14`)_
  don't use the process environment when launching
  processes, instead use only the environment specified. Be aware though that
  you will need to take into account things such as session specific variables.
  See <<ref_environment,environment blocks>> discussion.
...
`command_cwd`:: _(Available since Ninja 1.14 and only when
  `ninja_required_version >= 1.14`)_
  the directory that cwd will be set to when running the command. This is only
  supported if running on Windows, Linux (when
  posix_spawn_file_actions_addchdir_np is available) and macOS (10.15 and
  later).

[[ref_command_raw]] `command_raw`:: _(Available since Ninja 1.14 and only when
  `ninja_required_version >= 1.14`)_ the command will be executed directly with
  a process launch instead of going through a `/bin/sh -c` on posix. On Windows
  commands are always executed directly as a process launch, but this setting
  will allow uniform syntax for specifying command arguments and escape the
  process launch arguments correctly for Windows. For both Windows and posix the
  executable will not be located from `PATH`, a full path is always required.
  See <<ref_command_raw_parsing, raw command parsing>> for how arguments are
  parsed.

`environment`:: _(Available since Ninja 1.14 and only when
  `ninja_required_version >= 1.14`)_
  append these environment variables when launching processes. These environment
  variables are appended to system environment variables or globally specified
  environment variables if specified. See <<ref_environment,environment blocks>>
  discussion.
...
`build_priority`:: if present, provides a weight hint (in milliseconds)
  used by the critical path scheduler when prioritizing edges.  Ninja
  uses the greater of `build_priority` and the historical build time
  from `.ninja_log`.  When no historical data exists, `build_priority`
  is used instead of the default weight of 1.  Higher values cause the
  edge to be scheduled earlier.  This is useful for build system
  generators that know certain steps (e.g. linking) are expensive.
  Can be set on a rule (as a default for all edges using that rule) or
  on individual build statements (to override the rule default).
+
Use it like in the following example:
+
----
...
# Override rule default for a particularly expensive link
build myapp: link obj1.o obj2.o
  build_priority = 10000
----
...
Environment blocks
^^^^^^^^^^^^^^^^^^
Environment blocks specified in `environment` and `override_environment`
are formatted with one variable on each line. To escape new lines use `\n`.
To escape `\` use `\\`.

Example usage:
----
environment = PATH=C:\\Executables$^$
MY_ENV_VAR=My Value

rule link
  command = link.exe /OUT$out $in
  environment = LIB=C:\\Libraries$^$
PATH=C:\\LinkExecutables
----

For `override_environment` you can also specify lines with only the environment
variable name without the `=`. In this case the environment variable
will be copied from the environment ninja is running under:

----
override_environent = PATH$^$
HOME$^$
LANG$^$
XDG_SESSION_ID$^$
TMP$^$
...
The contents of environment block for each process launch is included in the
the build log hash for a command, so ninja can know to rebuild if the
environment block changes. The system environment is not included in this
calculation, so if you want to account for all environment variable changes
you should use `override_environment` so the environment variables are
...
To use uniform quoting that works both on unix and automatically converts to the
C library quoting on Windows see <<ref_command_raw, `command_raw`>>.
...
[[ref_command_raw_parsing]]
Raw command parsing
^^^^^^^^^^^^^^^^^^^
Arguments when parsing raw commands are split by space and strings can be quoted
with `"` or `'`. The escape character `\` is used for escaping quotes. Use `\"`
to escape quotes in double quoted strings, and `\'` to escape quotes in single
quoted strings. To escape `\` use `\\`.

Unix example:
----
rule echo
  command = /bin/echo Test "Test \" Test" 'Test \' Test'
  command_raw = true
----
...
----
rule echo
  command = "C:\\Unix Emu\\echo.exe" Test "Test \" Test" 'Test \' Test'
  command_raw = true
----
