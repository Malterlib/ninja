...
struct NinjaMain : public BuildLogUser {
  ...
  /// Take an exclusive lock on the build directory for the life of the
  /// process, so no second ninja runs in it: two processes appending to
  /// .ninja_log and .ninja_deps corrupt them, and the recovery on the next
  /// load then rebuilds everything. A held lock fails instead of waiting.
  /// The lock belongs to the process rather than to this instance: a manifest
  /// reload constructs a new NinjaMain and keeps the lock it already holds.
  /// @return false when another process holds the lock or it cannot be taken.
  bool LockBuildDir();
...
"  --quiet-success  suppress command output on success\n"
"\n"
...
std::string EvaluateCommandWithRspfile(const Edge* edge,
  ...
  string rspfile_content = edge->GetBinding("rspfile_content", false);
...
void PrintCompdbObjectsForEdge(std::string const& directory, const Edge* const edge,
  ...
  const auto& command = EvaluateCommandWithRspfile(edge, eval_mode, version);
  bool first = true;
...
int NinjaMain::ToolCompilationDatabase(const Options* options, int argc,
  ...
  for (const Edge* edge : state_.edges_) {
    ...
    if (argc == 0) {
      ...
      PrintCompdbObjectsForEdge(directory, edge, eval_mode, state_.minimum_version_);
      first = false;
    } else {
      for (int i = 0; i != argc; ++i) {
...
void PrintCompdb(std::string const& directory, std::vector<Edge*> const& edges,
  ...
  for (const Edge* edge : edges) {
    ...
    if (!first)
      putchar(',');
    PrintCompdbObjectsForEdge(directory, edge, eval_mode, version);
    first = false;
  }
...
int NinjaMain::ToolCompilationDatabaseForTargets(const Options* options,
  ...
  case CompdbTargets::Action::kEmitCommands: {
    ...
    std::string directory = GetWorkingDirectory();
    PrintCompdb(directory, collector.in_edges, compdb.eval_mode, state_.minimum_version_);
  } break;
  }
...
// The build lock of this process. real_main constructs a NinjaMain per manifest
// reload, so the lock lives here rather than in the instance: a reload that
// stays in the same build directory keeps the lock it holds instead of failing
// against its own earlier one, and a reload that moved the build directory
// releases it and takes a new one. Released when the process exits.
struct BuildDirLock {
#ifdef _WIN32
...
#ifdef _WIN32
  // No sharing at all: a second open fails with a sharing violation while
  // this handle is open, and the file goes away with the last handle.
...
int ReadFlags(int* argc, char*** argv,
  ...
  enum { OPT_VERSION = 1, OPT_QUIET = 2, OPT_QUIET_SUCCESS = 3 };
  const option kLongOptions[] = {
    { "help", no_argument, NULL, 'h' },
    { "version", no_argument, NULL, OPT_VERSION },
    { "verbose", no_argument, NULL, 'v' },
    { "quiet", no_argument, NULL, OPT_QUIET },
    { "quiet-success", no_argument, NULL, OPT_QUIET_SUCCESS },
    { NULL, 0, NULL, 0 }
  };
  ...
  while (!options->tool &&
    ...
    switch (opt) {
      ...
      case OPT_QUIET:
        ...
        break;
      case OPT_QUIET_SUCCESS:
        config->quiet_success = true;
        break;
      case 'w':
...
NORETURN void real_main(int argc, char** argv) {
  ...
  for (int cycle = 1; cycle <= kCycleLimit; ++cycle) {
    ...
    // Before the logs are opened: loading them already rewrites them.
